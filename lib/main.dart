...
class _MyAppState extends State<MyApp> {
  int cycleView = -1;    //Stores the current display cycle. -1 is the welcome screen.

  void updateDetailedView(val){    //Change current cycle and update view
    setState(() {
   ...
   getDataAPI(userKey) async{    //Get data from the API
    if (userKey != null) {
      var response = await http.get('https://rm9997.pythonanywhere.com/getUseableData/${userKey}');
      //If user exists. Reset cycle view and load new user data.
      if (response.body != 'Does not exist' && '${response.body[0]}' != '<') {
        print('New user showing: $userKey');
        setState(() {
          data = response.body;
          cycleView = -1;
        });
  ...
  Widget build(BuildContext context) {
    if (alreadyLoaded == 0) {    //Load 7890 as default user
      getDataAPI('7890');
      alreadyLoaded = 1;    //Change alreadyLoaded to show that non-default data has been loaded.
    }
    ...
    return MaterialApp(
      ...
      home: Scaffold(
          ...
          body: Center(
            child: Column(
              children: <Widget>[
                Container(child: MainView(barChooseFunc: updateDetailedView, parsedData: data)),    //Barchart view
                Container(child: DetailerView(cycleNo: cycleView, parsedData: parsedData['cycle$cycleView'])),    //Lower view
              ],
            ),
          )),
    );
